...
describe("multi-clasp versions -> execShellCommand", () => {
  const realArgv = process.argv.slice();

  beforeEach(() => {
    process.argv = ["node", "multi-clasp", "versions"];
  ...
  it("executes npx clasp versions", async () => {
    ...
    expect(execMock.mock.calls[0][0]).toBe("npx clasp versions ");
    expect(execMock.mock.calls[1][0]).toBe("npx clasp versions ");
  });
