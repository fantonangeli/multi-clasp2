...
describe("multi-clasp status -> execShellCommand", () => {
  const realArgv = process.argv.slice();

  beforeEach(() => {
    process.argv = ["node", "multi-clasp", "status"];
  ...
  it("no options: executes npx clasp status", async () => {
    ...
    expect(execMock.mock.calls[0][0]).toBe("npx clasp status ");
    expect(execMock.mock.calls[1][0]).toBe("npx clasp status ");
  });

  it("--json: executes npx clasp status --json", async () => {
    const tasks = mockForeach(["AAA", "BBB"]);
    ...
    expect(execMock).toHaveBeenCalledTimes(2);
    expect(execMock.mock.calls[0][0]).toBe("npx clasp status --json");
    expect(execMock.mock.calls[1][0]).toBe("npx clasp status --json");
  });
